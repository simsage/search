...
export function SourceSelector(props) {
    ...
    const {
        source_values, source_list, source_filter, busy
    ...
    function get_source_list() {
        ...
        source_group_list = source_group_list.sort((a, b) => (a.type > b.type) ? -1 : 1)
            .filter((item) => { return trim_filter.length === 0 ||
    ...
    return (
        <div className="category-selector list-group pt-1">
            ...
            <label className="list-group-item p-0 overflow-hidden">
                <input type="text" value={source_filter} placeholder="Filter type..." className="py-2 px-3 w-100 border-0"
                       disabled={busy}
                       readOnly={busy}
                        ...
                        <div className="list-group-item bg-light d-flex ps-3 pe-3 no-select" key={i}>
                            ...
                            <div className="d-flex justify-content-between flex-fill">
                                        <span title={"filter search results for only " + item.name + " sources."}><span>
                                            <img className="sourceTypeIcon" src={get_icon_path(item.name)} alt="search"/>
                                        </span>{item.name}</span>
